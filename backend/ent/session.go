...
func (*Session) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case session.FieldUserAgent:
			values[i] = session.ValueScanner.UserAgent.ScanValue()
		case session.FieldIP:
			values[i] = session.ValueScanner.IP.ScanValue()
		default:
...
func (_m *Session) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case session.FieldIP:
			if value, err := session.ValueScanner.IP.FromValue(values[i]); err != nil {
				return err
			} else {
				_m.IP = value
			}
