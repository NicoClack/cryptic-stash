...
type SessionQuery struct {
	...
	withElevationPasskey *PasskeyQuery
...
// QueryElevationPasskey chains the current query on the "elevationPasskey" edge.
func (_q *SessionQuery) QueryElevationPasskey() *PasskeyQuery {
	query := (&PasskeyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *SessionQuery) Clone() *SessionQuery {
	...
	return &SessionQuery{
		...
		withElevationPasskey: _q.withElevationPasskey.Clone(),
...
// WithElevationPasskey tells the query-builder to eager-load the nodes that are connected to
// the "elevationPasskey" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SessionQuery) WithElevationPasskey(opts ...func(*PasskeyQuery)) *SessionQuery {
	query := (&PasskeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withElevationPasskey = query
	return _q
}
...
func (_q *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := _q.withElevationPasskey; query != nil {
		if err := _q.loadElevationPasskey(ctx, query, nodes, nil,
			func(n *Session, e *Passkey) { n.Edges.ElevationPasskey = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SessionQuery) loadElevationPasskey(ctx context.Context, query *PasskeyQuery, nodes []*Session, init func(*Session), assign func(*Session, *Passkey)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Session)
	for i := range nodes {
		if nodes[i].ElevationPasskeyID == nil {
			continue
		}
		fk := *nodes[i].ElevationPasskeyID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "elevationPasskeyID" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *SessionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withElevationPasskey != nil {
			_spec.Node.AddColumnOnce(session.FieldElevationPasskeyID)
		}
	}
